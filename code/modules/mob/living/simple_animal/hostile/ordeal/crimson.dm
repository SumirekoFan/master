...
/mob/living/simple_animal/hostile/ordeal/crimson_clown/gib()
	//Please dont spam the animation sir
	if(being_gibbed)
		return
	being_gibbed = TRUE
	//If this was called from death then we would already be dead.
	if(stat != DEAD)
		death(TRUE)
...
//This proc offically destroys us.
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/proc/ReleaseTheClowns()
	...
	for(var/i = 1 to mob_spawn_amount)
		...
		var/mob/living/simple_animal/hostile/ordeal/nc = new clown_derivitive(T)
		if(istype(nc, /mob/living/simple_animal/hostile/ordeal/crimson_clown))
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_dusk
	...
	mob_spawn_amount = 2
	clown_derivitive = /mob/living/simple_animal/hostile/ordeal/crimson_noon
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_dusk/proc/do_roll(move_dir)
	charging = TRUE
	var/been_hit = list()
	for(var/times_ran = 1 to roll_num)
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/Destroy()
	Honknihilation()
	return ..()
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/gib()
	//If this was called from death then we would already be dead.
	if(stat != DEAD)
		death(TRUE)
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/proc/SpawnClown(turf/spawn_loc)
	...
	var/mob/living/simple_animal/hostile/ordeal/nb = new picked_mob(spawn_loc)
	RegisterSignal(nb, list(COMSIG_LIVING_DEATH, COMSIG_PARENT_QDELETING), PROC_REF(FireClown))
	LAZYADD(spawned_mobs, nb)
	if(ordeal_reference)
		nb.ordeal_reference = ordeal_reference
		ordeal_reference.ordeal_mobs += nb

/mob/living/simple_animal/hostile/ordeal/crimson_tent/proc/FireClown(mob/living/honk)
	UnregisterSignal(honk, list(COMSIG_LIVING_DEATH, COMSIG_PARENT_QDELETING))
	spawned_mobs -= honk

/mob/living/simple_animal/hostile/ordeal/crimson_tent/proc/Honknihilation()
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/proc/Bite(target)
	...
	var/turf/area_of_effect = FormulateBitingProcedure(dir_to_target)
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/proc/FormulateBitingProcedure(dir_to_target)
	. = list()
	...
	if(dir_to_target in directional_list)
		middle_line = getline(get_step_towards(source_turf, target), get_ranged_target_turf(source_turf, dir_to_target, bite_length))
		for(var/turf/T in middle_line)
			if(T.density)
				break
			for(var/turf/Y in getline(T, get_ranged_target_turf(T, right_dir, bite_width)))
				if (Y.density)
					break
				if (Y in .)
					continue
				. += Y
			for(var/turf/U in getline(T, get_ranged_target_turf(T, left_dir, bite_width)))
				if (U.density)
					break
				if (U in .)
					continue
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_midnight //oh god why
	...
	damage_coeff = list(RED_DAMAGE = 0.2, WHITE_DAMAGE = 1, BLACK_DAMAGE = 1, PALE_DAMAGE = 1.5)
	mob_spawn_amount = 2
	clown_derivitive = /mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_dusk/spawned
...
/mob/living/simple_animal/hostile/ordeal/crimson_clown/spawned/Initialize() //this should effectively limit how many are active at a time
	. = ..()
	//50% chance to not leave behind some items we have to worry about.
	if(prob(50))
		drop_meat = FALSE
	time_to_explode = world.time + (60 SECONDS)
...
/mob/living/simple_animal/hostile/ordeal/crimson_clown/spawned/Life()
	. = ..()
	if(!.)
		return
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/spawned/Initialize()
	. = ..()
	time_to_explode = world.time + (45 SECONDS)
	animate(src, transform = matrix()*1.2, color = "#FF0000", time = 45 SECONDS)

/mob/living/simple_animal/hostile/ordeal/crimson_noon/spawned/Life()
	. = ..()
	if(!.)
		return
	if(world.time >= time_to_explode)
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_dusk/spawned/Life()
	. = ..()
	if(!.)
		return
	if(world.time >= time_to_explode)
