...
/datum/action/spell_action/spell/rca_axe_throw/IsAvailable()
	if (istype(owner, /mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman))
...
/obj/effect/proc_holder/spell/pointed/rca_axe_throw
	...
	base_action = /datum/action/spell_action/spell/rca_axe_throw


/obj/effect/proc_holder/spell/pointed/rca_axe_throw/cast(list/targets, mob/user)
	var/target = targets[1]

	if (istype(user, /mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman))
		var/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/W = user
		if (W.chained_target)
			return FALSE

	var/obj/projectile/rca_chainedaxe/P = new(get_turf(user))
	P.firer = user
	P.preparePixelProjectile(target, user)
	P.fire()

/obj/projectile/rca_chainedaxe
	name = "chained axe"
...
/obj/projectile/rca_chainedaxe/on_hit(atom/target, blocked = FALSE)
	...
	if(istype(target, /mob/living/carbon/human))
		...
		var/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/W = firer
		if(istype(W) && get_dist(get_turf(target), get_turf(firer)) < 12)
			W.begin_chain_pull(H)

/datum/action/innate/rca_abnormality_attack/toggle/woodsman_flurry_toggle
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/proc/begin_chain_pull(mob/living/carbon/human/target)
	...
	if(!C)
		C = chained_target.apply_status_effect(/datum/status_effect/rca_chained)
		C.W = src
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/proc/pull_loop()
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/proc/update_chain_visuals()
	if(!chained_target)
		if(chain_beam)
...
/atom/movable/screen/alert/status_effect/rca_chained
	name = "Chained"
	desc = "You've been caught by the woodsman's chain! Break line of sight before it's too late!"
	icon = 'ModularLobotomy/_Lobotomyicons/status_sprites.dmi'
	icon_state = "locked"

// Movement restriction for chained targets
/datum/status_effect/rca_chained/on_apply()
	. = ..()
...
/datum/status_effect/rca_chained/proc/check_movement(mob/living/carbon/human/H, turf/NewLoc)
	...
	if(!W || !(H in view(view_range, W)))
		H.remove_status_effect(/datum/status_effect/rca_chained)
		return
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/Life()
	. = ..()
	if(!(ramping == 0) && ramping_decay <= world.time)
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/Move()
	if(!can_act)
		return FALSE
	return ..()

/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/AttackingTarget(atom/attacked_target)
	if(!can_act)
		return FALSE
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/PickTarget(list/Targets) // We attack corpses first if there are any
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/proc/GainRamping(amount)
	ramping += amount
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/proc/RampingUpdate()
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/proc/Heal(mob/living/carbon/human/body)
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/CanAttack(atom/the_target)
...
/mob/living/simple_animal/hostile/rcorp_abno/easy/woodsman/OpenFire()
	if(!can_act)
		return FALSE
	if(client)
		switch(chosen_attack)
