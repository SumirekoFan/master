...
/obj/item/food/fish
	...
	desc = "very bland"
	icon = 'ModularLobotomy/fishing/icons/fish_sprites.dmi'
	icon_state = "bugfish"
	...
	// Fishing Catalogue lists this as a place you can fish them from.
	var/habitat = "Unknown"



/obj/item/food/fish/Initialize(mapload)
	. = ..()
	if(fillet_type)
...
/obj/item/food/fish/examine(mob/user)
	. = ..()
	// All spacemen have magic eyes of fish weight perception until fish scale (get it?) is implemented.
	. += "It's [size] cm long."
	. += "It weighs [weight] g."

/obj/item/food/fish/proc/randomize_weight_and_size(modifier = 0)
...
/obj/item/food/fish/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change = TRUE)
	. = ..()
	check_environment_after_movement()

/obj/item/food/fish/proc/enter_stasis()
	in_stasis = TRUE
	// Stop processing until inserted into aquarium again.
	stop_flopping()
	STOP_PROCESSING(SSobj, src)

/obj/item/food/fish/proc/exit_stasis()
	in_stasis = FALSE
	if(status != FISH_DEAD)
		START_PROCESSING(SSobj, src)
...
/obj/item/food/fish/proc/attack_reaction(datum/source, obj/item/thing, mob/user, params)
	SIGNAL_HANDLER
	if(is_food(thing))
...
/obj/item/food/fish/proc/is_food(obj/item/thing)
	return istype(thing, /obj/item/fish_feed)

/obj/item/food/fish/proc/on_feeding(datum/reagents/feed_reagents)
	if(feed_reagents.has_reagent(food))
		last_feeding = world.time

/obj/item/food/fish/proc/check_environment_after_movement()
...
/obj/item/food/fish/process(seconds_per_tick)
...
/obj/item/food/fish/proc/set_status(new_status)
	switch(new_status)
		if(FISH_ALIVE)
			status = FISH_ALIVE
...
/obj/item/food/fish/proc/get_aquarium_animation()
...
/obj/item/food/fish/proc/proper_environment()
	var/obj/structure/aquarium/aquarium = loc
	if(!istype(aquarium))
		return FALSE
...
/obj/item/food/fish/proc/process_health(seconds_per_tick)
	var/health_change_per_second = 0
	if(!proper_environment())
		health_change_per_second -= 5 //Dying here
...
/obj/item/food/fish/proc/adjust_health(amt)
	health = clamp(amt, 0, initial(health))
	if(health <= 0)
		set_status(FISH_DEAD)


/obj/item/food/fish/proc/ready_to_reproduce()
	var/obj/structure/aquarium/aquarium = loc
	if(!istype(aquarium))
		return FALSE
...
/obj/item/food/fish/proc/try_to_reproduce()
	var/obj/structure/aquarium/aquarium = loc
	if(!istype(aquarium))
		return
	...
	for(var/obj/item/food/fish/fish_in_aquarium in aquarium)
	...
	if(length(other_fish_of_same_type) >= stable_population)
		return
	var/obj/item/food/fish/second_fish
	for(var/obj/item/food/fish/other_fish in other_fish_of_same_type)
