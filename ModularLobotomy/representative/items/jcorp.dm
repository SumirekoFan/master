...
/obj/item/a_gift/jcorp/get_gift_type()
	// If our loot list isn't generated yet...
	if(!GLOB.possible_loot_jcorp.len)
		// Okay I'm refactoring this to be procedurally generated because it used to be a 375 line hardcoded list.
		var/list/gift_types_list = list()
		// Find all lootcrate subtypes.
		for(var/lootcrate_path in subtypesof(/obj/structure/lootcrate))
			// I DESPISE having to do this but we can't access list contents of uninstantiated things. Setting crate_type to lootcrate_path, raw, lets us access stuff like the droprate, but the lootpools are uninitialized lists. We delete this later.
			var/obj/structure/lootcrate/crate_type = new lootcrate_path()

			// Don't add contents of the money crate or Tres crate, or other verboten crates.
			if(crate_type.jcorp_banned)
				continue

			// Now, add the lootcrate subtype's common, rare and very rare lootpools to our gift types list. We use |= here so there are NO DUPLICATES. So, if Zweihander shows up in Hana & Zwei crate, it will only count ONCE.
			// Feel free to change it to += in which case rewards that appear in multiple crates are likelier to appear. Either is fine by me honestly. I just think it feels more random this way, which is very J-Corp pilled.
			for(var/common_item in crate_type.lootlist)
				gift_types_list |= common_item
			for(var/rare_item in crate_type.rareloot)
				gift_types_list |= rare_item
			for(var/veryrare_item in crate_type.veryrareloot)
				gift_types_list |= veryrare_item

			// Keen-eyed lootcrate enjoyers may notice the Cosmetic lootpool is missing here. Uh... add it if you want? I think it's a bit mean

			qdel(crate_type) // Goodbye and good riddance

		for(var/V in gift_types_list)
			var/obj/item/I = V
			if(istype(I) && ((!initial(I.icon_state)) || (!initial(I.inhand_icon_state)) || (initial(I.item_flags) & (ABSTRACT | DROPDEL)))) // Remove items which match these criteria from our list. Keep non-item stuff in there (mobs?)
				gift_types_list -= V

		GLOB.possible_loot_jcorp = gift_types_list // There, now our loot list is generated and we don't need to generate it again.
